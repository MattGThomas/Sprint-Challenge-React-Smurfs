...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
      ...
      Client side routing allows full webpage to be loaded up front, which in turn allows javascript to handle routing from page to page. server side routing will send a request to the server, which in turn sends information back to the client, this process forces a refresh of the web page, and also does not give us access to state.
